...
router.get('/:id', async (req, res) => {
    ...
    if (req.query.history === 'true') {
        station = await Station.findById(req.params.id).select('-fuels.history._id -address._id -coordinates._id').lean();
    } else {
        station = await Station.findById(req.params.id).select('-fuels.history -address._id -coordinates._id').lean();  // -> by default, do not return history path on query if not explicitly specified
    }
